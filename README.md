...
项目目前处于从零重建的实验阶段，已完成 M0-S001—M0-S024，当前设计基线为 S001 3.5，产品 schema 为 v12。唯一认知模型绑定是火山方舟 `doubao-seed-evolving`；它是服务端持续演进的统一模型入口，因此每次 attempt 都会保管实际回显的模型身份，不能把跨升级输出冒充成固定版本复现。设计已补齐 candidate v2 的事实类别、owner、base version、原子依赖组和确定性校验边界，但 S025 尚未实施。生产链已有真实 Responses 调用、模型制品、attempt、预算、租约续期和恢复；仓库和长期环境仍没有默认人格、已出生主体、活动 Runtime、真实 Creator 凭据或业务数据，因此还不是可用产品版本，模型结果仍不能直接改变主体。
