...
| 物理 schema | 当前 baseline 108 张表、65 个显式索引；表和生产 DML 都受 owner registry 检查 |
...
这些命令会连接或修改目标本地环境，执行前应核对绝对路径和 credential locator。完整环境、QQ、音视频、备份、恢复、维护与重置手册见 [安装、启动与维护](docs/04-运行与验证/01-安装、启动与维护.md)。
...
该 gate 会产生真实对话记录和模型调用，并验证 cognition、Subject Commit、reply Effect、outbox 和回复 artifact。详细 gate 边界见 [质量门禁与 Live 验证](docs/04-运行与验证/02-质量门禁与Live验证.md)。
...
- [系统设计](docs/02-系统设计/)：权威运行时、认知、数据、权限/效果、恢复、Mood。
- [实现参考](docs/03-实现参考/)：模块、数据库、配置、接口、模型/Codex/渠道、设备。
- [运行与验证](docs/04-运行与验证/)：运行手册、质量门禁和实测性能基线。
