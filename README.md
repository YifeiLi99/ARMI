...
心情链路为事件 → Jev 事实评价 → 本地心理算法 → Mood 独立提交 → 主认知。Jev 使用独立 `mood.jev_api_key` 凭据槽，固定 `jev-1.13.0`；缺失或失败时停止该事件依赖的认知，无替代模型。具体规则见 [Mood 设计](DESIGN.md#8-mood)，合成实验见 [运行入口](tools/jev-experiment.md#新-mood-连续事件实验)。本轮数据库 baseline 不兼容旧库，普通启动不迁移已有主体。
