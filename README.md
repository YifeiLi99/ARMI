...
项目目前处于从零重建的实验阶段，已完成 M0-S001—M0-S005；公共 transport `"1.0"` 已作为 Kernel 的严格、不可变工程契约建立，并由共享 Python/TypeScript wire 向量验证。当前仍只有可复现工程底座，没有产品能力或可运行版本；M0-S006 仅可进入计划模式，尚未授权配置或秘密入口实施。
