...
项目目前处于从零重建的实验阶段，已完成 M0-S001—M0-S006；严格的 `armi.runtime-config.v1`、三层配置合并、脱敏摘要、路径预检与 `env:`/`file:` 秘密入口已经建立。当前仍只有可复现工程底座，没有 OpenAPI、数据库 schema、产品能力、服务入口或可运行版本；M0-S007 仅可进入计划模式，尚未授权 OpenAPI、前端静态壳或 Runtime 实施。
