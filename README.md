...
项目目前处于从零重建的实验阶段，已完成 M0-S001—M0-S011，并将 S001 升至 1.9。PostgreSQL 18.4 最小 baseline、环境作用域角色和最小 grant 继续成立；Runtime 新增了不会自动打开的异步 Unit of Work、固定锁计划、CAS 判定、提交前挂点、不可变提交后动作和脱敏 SQLSTATE 错误边界。当前没有生产业务表、repository 或权威状态，Runtime 启动也不会打开业务连接池；恢复、审计、浏览器会话和产品能力尚未实现，Runtime 保持 blocked/not-ready，因此不是可用产品版本。M0-S012 仅可进入计划模式，不代表已授权创建制品表或文件存储。
