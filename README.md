...
项目目前处于从零重建的实验阶段，已完成 M0-S001—M0-S012，并将 S001 升至 2.0。当前已建立 PostgreSQL 18.4 下的 `armi.artifacts` 权威元数据、按 SHA-256 寻址的本机文件正文、原子发布与登记协调、逐次完整校验读取和只读孤儿报告；环境作用域角色、列级 grant 与异步 Unit of Work 继续成立。制品组件尚未接入 Runtime 启动，系统也仍无审计、权威业务状态、恢复、浏览器会话或产品能力；Runtime 保持 blocked/not-ready，因此不是可用产品版本。M0-S013 仅可进入计划模式。
