...
项目目前处于从零重建的实验阶段，已完成 M0-S001—M0-S011，并将 S001 升至 2.0，冻结了内容寻址制品的元数据、重复内容、崩溃顺序、verified read 和孤儿报告边界。PostgreSQL 18.4 最小 baseline、环境作用域角色、最小 grant 与异步 Unit of Work 继续成立。当前尚未创建 artifact 表、文件存储接线、权威业务状态、恢复、审计、浏览器会话或产品能力，Runtime 保持 blocked/not-ready，因此不是可用产品版本。M0-S012 已获单独实施授权。
