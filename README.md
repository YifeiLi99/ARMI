...
项目目前处于从零重建的实验阶段，已完成 M0-S001—M0-S012，并将 S001 升至 2.1，冻结正常审计的固定字段、安全引用、同事务规则、精确查询和诊断降级语义。当前已建立 PostgreSQL 18.4 下的 `armi.artifacts` 权威元数据、按 SHA-256 寻址的本机文件正文、原子发布与登记协调、逐次完整校验读取和只读孤儿报告；环境作用域角色、列级 grant 与异步 Unit of Work 继续成立。`armi.audit_events` 及其 Runtime 接线尚未实施，系统也仍无权威业务状态、恢复、浏览器会话或产品能力；Runtime 保持 blocked/not-ready，因此不是可用产品版本。M0-S013 已获准按冻结设计实施。
