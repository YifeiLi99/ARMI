...
项目目前处于从零重建的实验阶段，已完成 M0-S001—M0-S009，并建立了 PostgreSQL 18.4 最小 baseline、schema manifest、固定迁移入口和 Runtime 只读 schema 核验。当前数据库只包含 `armi` schema 与 `armi.schema_migrations`；业务表、正式角色权限、权威状态、恢复、审计、浏览器会话和产品能力均未实现，Runtime 仍保持 blocked/not-ready，因此不是可用产品版本；M0-S010 仅开放计划前置条件。
