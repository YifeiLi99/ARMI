...
项目目前处于从零重建的实验阶段，已完成 M0-S001—M0-S025，当前设计基线为 S001 3.5，产品 schema 为 v13。唯一认知模型绑定是火山方舟 `doubao-seed-evolving`；它是服务端持续演进的统一模型入口，因此每次 attempt 都会保管实际回显的模型身份，不能把跨升级输出冒充成固定版本复现。生产链现已具备真实 Responses 调用、模型制品、attempt、预算、租约续期、candidate v2 严格解析、确定性领域校验和未生效 `SubjectChangeSet` 保管；Memory、Relationship、Activity、Capability 与 Action owner 仍未激活。仓库和长期环境没有默认人格、已出生主体、活动 Runtime、真实 Creator 凭据或业务数据，因此还不是可用产品版本；候选校验成功也不会直接改变主体，S026 仍须重新核验当前 fence 与版本后执行权威提交。
