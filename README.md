...
项目目前处于从零重建的实验阶段，已完成 M0-S001—M0-S013，当前 S001 2.1 设计基线保持冻结。PostgreSQL 18.4 已建立追加式 `armi.audit_events` 正常审计底座；首次 artifact 元数据登记及首次完整性降级会在同一事务中写入固定安全字段，完全重复登记不会制造第二条事实，精确只读查询也未接入产品入口。文件诊断失败会以脱敏原因降级到 stderr，但不会补写或改变数据库事实。系统仍无 durable work、权威主体状态、恢复、浏览器会话或产品能力；Runtime 因 S016—S018 前置缺失继续保持 blocked/not-ready，因此不是可用产品版本。M0-S014 仅可进入计划模式，未自动授权实施。
