...
项目当前已达到 M0-Core 单机个人内测可用；P0-S001—S011 的 Activity、注意、Creator 只读闭环、睡眠维护、主观记忆演进、精确生活查询、关系边界和承诺/冲突连续已经落地，当前按功能优先模式推进 P0-S012。普通 `consider_creator_input` 仍只调用一次主模型：模型返回本轮决定、必要正文、可选 Experience，以及最多一项当前记忆变化和一项由同轮 Experience 支持的关系变化。Runtime 绑定 memory/relationship/commitment head、参与方、来源、版本和权限；承诺不是权限或强制执行脚本，违背和矛盾不会被新的 current state 抹除，跨 scene 延续当前承诺时也不复制近期现场原文。关系不会由单一分数、隐藏阈值或预设亲子/友情/爱情产生，结束联系的当前边界会阻止后续 Creator 回复。精确查询明确标记为本次取得的记录证据，不会把已经遗忘的内容伪装成自然回忆。2026-08-04 的当前环境实测 reply effect 为 `completed/verified`，ARMI 回复“可以啦，我们现在就在正常对话中。”
...
不再阻塞后续功能开发。P0-S001—P0-S011 按功能实现完成，当前施工入口为 P0-S012。
