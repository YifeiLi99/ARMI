...
Creator 与其他人的入站对话共用版本化的紧凑 Prompt 计划：Runtime 按领域渲染必要状态，只把可被本轮变化引用的对象标为 `ctx:*`，近期对话以原生 `user/assistant` 消息保留完整轮次。普通对话仍只调用一次主模型；同一次严格结构化输出同时承载自然表达、必要 Experience 和最多八项真实变化。输出 Schema 使用稳定的紧凑 `changes` 合同，实际引用、权限、版本和领域组合继续由 Runtime 确定性校验。
