...
项目目前处于从零重建的实验阶段，已完成 M0-S001—M0-S024，当前设计基线为 S001 3.4，产品 schema 为 v12。唯一认知模型绑定是火山方舟 `doubao-seed-evolving`；它是服务端持续演进的统一模型入口，因此每次 attempt 都会保管实际回显的模型身份，不能把跨升级输出冒充成固定版本复现。生产链已具备 provider-neutral request、真实 Responses 调用、模型 request/response 制品、attempt 账本、预算、租约续期和恢复边界；没有模型密钥时不会领取模型工作，Runtime 以 `degraded/ready` 暴露能力不可用。仓库和长期环境仍没有默认人格、已出生主体、活动 Runtime、真实 Creator 凭据或业务数据，因此还不是可用产品版本；S024 的模型结果仍只是未获信任的认知候选，领域校验和主体提交分别属于后续步骤。
