...
项目目前处于从零重建的实验阶段，已完成 M0-S001—M0-S018，并已将 M0-S019 的默认 scene、耐久 timeline、稳定游标和权威 Query 契约重冻为 S001 2.8；产品 schema 仍为 v8，S019 实现尚未完成。born Runtime 现在可在取得 authority、完成恢复并接好 Creator credential 后进入 `ready`，并通过严格同源 loopback 提供一次性 bootstrap code、短期 browser session、安全运行状态和最小 Creator 会话壳；这些会话完全驻进程内存，drain、到期、注销或重启即失效。仓库和长期环境没有默认人格、已出生主体、活动 Runtime、真实 Creator 凭据、scene、业务 timeline、输入页面或后台 worker，仍不是可用产品版本。
