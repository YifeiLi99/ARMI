-- Frozen ARMI v1 activities and maintenance.
...
CREATE TABLE armi.activity_decisions (
    ...
    CONSTRAINT activity_decisions_source_check CHECK (decision_source IN ('attention', 'internal_work')),
    CONSTRAINT activity_decisions_head_version_check CHECK (expected_head_version > 0),
    CONSTRAINT activity_decisions_digest_check CHECK (resource_snapshot_digest ~ '^sha256:[0-9a-f]{64}$'),
    CONSTRAINT activity_decisions_shape_check CHECK ((decision_source = 'attention' AND output_material_id IS NULL) OR (decision_source = 'internal_work' AND review_not_before IS NULL AND result_revision_id IS NOT NULL)),
    CONSTRAINT activity_decisions_schema_version_check CHECK (schema_version = 1)
);
