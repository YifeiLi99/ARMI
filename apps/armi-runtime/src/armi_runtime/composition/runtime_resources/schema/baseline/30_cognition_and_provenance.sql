-- Frozen ARMI v1 cognition and provenance.
...
CREATE TABLE armi.cognitive_episodes (
    ...
    CONSTRAINT cognitive_episodes_scene_shape_check CHECK (((purpose IN ('consider_autonomous_life', 'consider_activity_attention', 'consider_activity_internal_work', 'consider_sleep', 'maintain_subjective_memory', 'perform_subject_self_check') AND scene_id IS NULL AND context_party_id IS NULL) OR (purpose NOT IN ('consider_autonomous_life', 'consider_activity_attention', 'consider_activity_internal_work', 'consider_sleep', 'maintain_subjective_memory', 'perform_subject_self_check') AND scene_id IS NOT NULL AND context_party_id IS NOT NULL))),
...
CREATE TABLE armi.external_evidence (
    ...
    CONSTRAINT external_evidence_source_identity_check CHECK ((source_kind NOT IN ('creator_input', 'other_human_input') OR interaction_id IS NOT NULL) AND ((source_kind = 'web_search') = (web_observation_request_id IS NOT NULL AND observation_attempt_id IS NOT NULL)) AND ((source_kind = 'codex_task_source') = (codex_task_source_id IS NOT NULL)) AND ((source_kind = 'codex_result') = (codex_verification_id IS NOT NULL))),
...
CREATE TABLE armi.opportunities (
    ...
    CONSTRAINT opportunities_source_shape_check CHECK (((source_kind = 'external_evidence' AND evidence_id = source_ref AND scene_id IS NOT NULL AND context_party_id IS NOT NULL AND activity_id IS NULL) OR (source_kind IN ('life_generation_available', 'subject_component_revision', 'maintenance_window', 'maintenance_phase_revision', 'life_material_revision') AND evidence_id IS NULL AND scene_id IS NULL AND context_party_id IS NULL AND activity_id IS NULL) OR (source_kind = 'activity_revision' AND evidence_id IS NULL AND scene_id IS NULL AND context_party_id IS NULL AND activity_id IS NOT NULL) OR (source_kind IN ('life_query_result', 'creator_outreach_absence', 'creator_outreach_relationship') AND evidence_id IS NULL AND scene_id IS NOT NULL AND context_party_id IS NOT NULL AND activity_id IS NULL) OR (source_kind = 'creator_outreach_activity' AND evidence_id IS NULL AND scene_id IS NOT NULL AND context_party_id IS NOT NULL AND activity_id IS NOT NULL))),
