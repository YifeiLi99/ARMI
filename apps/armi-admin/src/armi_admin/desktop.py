...
_CREDENTIAL_NAMES = {
    ...
    "Jev API Key（心理评价与自主检查）": "mood.jev_api_key",
...
class Desktop:
    ...
    def _credential_selected(self, _event: object = None) -> None:
        ...
        if name == "model.ark_api_key":
            ...
            self.credential_help.set(
                "填写 TypeSafe 官方控制台的 API Key，用于每次事件的 Mood/Mind 评价，以及空闲时每分钟的自主检查。\n首次添加凭据配置需要重启 Runtime；之后替换 Key 在下一次 Jev 请求生效。此处只保存，不发起付费验证，也不代表服务已经可用。"
            )
            self.secret.pack(fill="x", before=self.credential_actions)
