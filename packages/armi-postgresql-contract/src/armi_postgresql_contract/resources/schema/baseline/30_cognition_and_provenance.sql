...
CREATE TABLE armi.cognitive_attempts (
    ...
    candidate_contract_kind text NOT NULL,
    ...
    CONSTRAINT cognitive_attempts_candidate_contract_kind_check CHECK (candidate_contract_kind = ANY (ARRAY['armi.autonomy-check'::text, 'armi.autonomous-activity-candidate'::text, 'armi.cognition-candidate'::text, 'armi.creator-cognitive-act-candidate'::text, 'armi.creator-voice-act-candidate'::text, 'armi.maintenance-work-candidate'::text, 'armi.other-human-dialogue-candidate'::text, 'armi.owner-reflection-candidate'::text, 'armi.sleep-decision-candidate'::text, 'armi.visual-observation-candidate'::text])),
    CONSTRAINT cognitive_attempts_check CHECK ((((dispatch_status = 'prepared'::text) AND (dispatched_at IS NULL) AND (settled_at IS NULL) AND (result_status IS NULL) AND (provider_request_id IS NULL) AND (provider_model_id IS NULL) AND (response_artifact_id IS NULL) AND (input_tokens IS NULL) AND (output_tokens IS NULL) AND (cached_input_tokens IS NULL) AND (error_code IS NULL)) OR ((dispatch_status = 'dispatched'::text) AND (dispatched_at IS NOT NULL) AND (settled_at IS NULL) AND (result_status IS NULL) AND (response_artifact_id IS NULL) AND (error_code IS NULL)) OR ((dispatch_status = 'settled'::text) AND (settled_at IS NOT NULL) AND (result_status IS NOT NULL) AND (((result_status = 'succeeded'::text) AND (dispatched_at IS NOT NULL) AND ((provider_request_id IS NOT NULL) OR provider='typesafe') AND (provider_model_id IS NOT NULL) AND (response_artifact_id IS NOT NULL) AND (input_tokens IS NOT NULL) AND (output_tokens IS NOT NULL) AND (cached_input_tokens IS NOT NULL) AND (error_code IS NULL)) OR ((result_status <> 'succeeded'::text) AND (response_artifact_id IS NULL) AND (error_code IS NOT NULL) AND ((dispatched_at IS NOT NULL) OR ((result_status = 'cancelled'::text) AND (provider_request_id IS NULL) AND (provider_model_id IS NULL) AND (input_tokens IS NULL) AND (output_tokens IS NULL) AND (cached_input_tokens IS NULL)))))))),
    CONSTRAINT cognitive_attempts_credential_identity_check CHECK (credential_identity IN ('mood.jev_api_key', 'armi.model.ark-api-key.v1', 'armi.model.qwen-api-key.v1', 'armi.model.deepseek-api-key.v1')),
    ...
    CONSTRAINT cognitive_attempts_profile_check CHECK ((profile = ANY (ARRAY['autonomy_check'::text, 'creator_input_cognition'::text, 'creator_cognitive_act'::text, 'creator_voice_act'::text, 'creator_outreach'::text, 'other_human_dialogue'::text, 'autonomous_activity'::text, 'activity_attention'::text, 'activity_internal_work'::text, 'sleep_decision'::text, 'memory_maintenance'::text, 'subject_self_check'::text, 'reflect_self'::text, 'reflect_focus'::text, 'reflect_prompt'::text, 'codex_task'::text, 'codex_result'::text, 'visual_observation'::text]))),
    CONSTRAINT cognitive_attempts_provider_check CHECK (provider IN ('typesafe', 'volcengine_ark', 'qwen', 'deepseek')),
...
CREATE TABLE armi.cognitive_episodes (
    ...
    CONSTRAINT cognitive_episodes_maintenance_result CHECK (maintenance_outcome IS NULL OR (
        ...
        OR (maintenance_phase IN ('reflect_self','reflect_focus','reflect_prompt') AND maintenance_outcome IN ('reflection_changed','reflection_unchanged')))
        ...
        AND (maintenance_issue_target IS NULL OR maintenance_issue_target IN ('self','focus','prompt'))
...
CREATE TABLE armi.context_embedding_coverage (
    ...
    CONSTRAINT context_embedding_coverage_binding_check CHECK ((model_binding = 'armi.embedding.qwen3-0_6b-q8_0-local-1024'::text)),
...
CREATE TABLE armi.context_embedding_projections (
    ...
    CONSTRAINT context_embedding_projections_binding_check CHECK ((model_binding = 'armi.embedding.qwen3-0_6b-q8_0-local-1024'::text)),
...
CREATE TABLE armi.event_appraisals (
    event_appraisal_id uuid PRIMARY KEY CHECK (uuid_extract_version(event_appraisal_id)=7),
    subject_id uuid NOT NULL,
    ...
    base_mind_version bigint NOT NULL CHECK (base_mind_version>0),
    mood_revision_id uuid,
    mind_revision_id uuid,
    mood_status text NOT NULL CHECK (mood_status IN ('running','applied','unchanged','failed')),
    mind_status text NOT NULL CHECK (mind_status IN ('running','applied','unchanged','failed')),
    mood_result jsonb,
    mind_result jsonb,
    mood_error text,
    mind_error text,
    ...
    CHECK (status NOT IN ('applied','unchanged') OR (
        mood_revision_id IS NOT NULL AND mind_revision_id IS NOT NULL
        AND mood_status IN ('applied','unchanged') AND mind_status IN ('applied','unchanged'))),
    CHECK (mood_status NOT IN ('applied','unchanged') OR mood_revision_id IS NOT NULL),
    CHECK (mind_status NOT IN ('applied','unchanged') OR mind_revision_id IS NOT NULL)
);
...
CREATE TABLE armi.opportunities (
    ...
    AND consideration_signals->>'schema_kind'='armi.consideration-signals'
