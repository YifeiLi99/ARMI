...
INSERT INTO armi.schema_baseline_identity DEFAULT VALUES;
...
CREATE TABLE armi.subject_component_revisions (
    ...
    CONSTRAINT subject_component_revisions_origin_check CHECK ((((origin_kind = 'bootstrap'::text) AND (component_version = 1) AND (previous_revision_id IS NULL) AND (subject_commit_id IS NULL) AND (proposal_ref IS NULL)) OR ((origin_kind = 'subject_commit'::text) AND (component_version > 1) AND (previous_revision_id IS NOT NULL) AND (subject_commit_id IS NOT NULL) AND (proposal_ref IS NOT NULL)) OR ((origin_kind = ANY (ARRAY['admin_correction'::text,'data_rights'::text,'event_appraisal'::text])) AND (component_version > 1) AND (previous_revision_id IS NOT NULL) AND (subject_commit_id IS NULL) AND (proposal_ref IS NULL)))),
    CONSTRAINT subject_component_revisions_origin_kind_check CHECK ((origin_kind = ANY (ARRAY['bootstrap'::text, 'subject_commit'::text, 'admin_correction'::text, 'data_rights'::text, 'event_appraisal'::text]))),
...
CREATE TABLE armi.mind_revisions (
    ...
    CONSTRAINT mind_revisions_origin_check CHECK ((origin_kind='bootstrap' AND mind_version=1 AND previous_revision_id IS NULL) OR (origin_kind IN ('event_appraisal','admin_correction','data_rights') AND mind_version>1 AND previous_revision_id IS NOT NULL)),
    CONSTRAINT mind_revisions_origin_kind_check CHECK (origin_kind IN ('bootstrap','event_appraisal','admin_correction','data_rights')),
...
CREATE TABLE armi.focus_revisions (
    is_current boolean DEFAULT false NOT NULL,
    focus_revision_id uuid NOT NULL,
    subject_id uuid NOT NULL,
    ...
    data_rights_redacted_at timestamp(6) with time zone,
    admin_change_id uuid,
    CONSTRAINT focus_revisions_focus_revision_id_check CHECK ((uuid_extract_version(focus_revision_id) = 7)),
    CONSTRAINT focus_revisions_focus_version_check CHECK ((focus_version > 0)),
    CONSTRAINT focus_revisions_admin_provenance CHECK (admin_change_id IS NULL OR origin_kind='admin_correction'),
    CONSTRAINT focus_revisions_origin_check CHECK ((((origin_kind = 'bootstrap'::text) AND (focus_version = 1) AND (previous_revision_id IS NULL) AND (subject_commit_id IS NULL) AND (proposal_ref IS NULL)) OR ((origin_kind = 'subject_commit'::text) AND (focus_version > 1) AND (previous_revision_id IS NOT NULL) AND (subject_commit_id IS NOT NULL) AND (proposal_ref IS NOT NULL)) OR ((origin_kind = ANY (ARRAY['admin_correction'::text,'data_rights'::text])) AND (focus_version > 1) AND (previous_revision_id IS NOT NULL) AND (subject_commit_id IS NULL) AND (proposal_ref IS NULL)))),
    CONSTRAINT focus_revisions_origin_kind_check CHECK ((origin_kind = ANY (ARRAY['bootstrap'::text, 'subject_commit'::text, 'admin_correction'::text,  'data_rights'::text]))),
    CONSTRAINT focus_revisions_origin_ref_check CHECK ((uuid_extract_version(origin_ref) = 7)),
    CONSTRAINT focus_revisions_proposal_ref_check CHECK (((proposal_ref IS NULL) OR (proposal_ref ~ '^proposal:[1-9][0-9]{0,2}$'::text))),
    CONSTRAINT focus_revisions_semantic_payload_check CHECK ((jsonb_typeof(semantic_payload) = 'object'::text))
);
