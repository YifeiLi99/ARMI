...
CREATE TABLE armi.relationship_revisions (
    ...
    CONSTRAINT relationship_revisions_mechanism_identity_check CHECK ((mechanism_identity = ANY (ARRAY['armi.relationship.admin'::text, 'armi.relationship.contextual'::text, 'armi.relationship.lifecycle'::text]))),
...
CREATE TABLE armi.subjective_memory_revisions (
    ...
    CONSTRAINT subjective_memory_revisions_check CHECK ((admin_change_id IS NOT NULL AND mechanism_identity='armi.memory.admin' AND mechanism_config_identity='admin' AND ((revision_no=1 AND previous_revision_id IS NULL AND revision_kind='formed') OR (revision_no>1 AND previous_revision_id IS NOT NULL AND revision_kind<>'formed'))) OR (((revision_no = 1) AND (previous_revision_id IS NULL) AND (revision_kind = 'formed'::text) AND (accessibility = 'available'::text) AND (mechanism_identity = 'armi.memory-formation.contextual'::text) AND (mechanism_config_identity = 'formation'::text)) OR ((revision_no > 1) AND (previous_revision_id IS NOT NULL) AND (revision_kind <> 'formed'::text) AND (mechanism_identity = 'armi.memory-revision.contextual'::text) AND (mechanism_config_identity = ANY (ARRAY['natural-dialogue'::text, 'sleep-maintenance'::text]))))),
    ...
    CONSTRAINT subjective_memory_revisions_mechanism_config_identity_check CHECK ((mechanism_config_identity = ANY (ARRAY['admin'::text, 'formation'::text, 'natural-dialogue'::text, 'sleep-maintenance'::text]))),
    CONSTRAINT subjective_memory_revisions_mechanism_identity_check CHECK ((mechanism_identity = ANY (ARRAY['armi.memory.admin'::text, 'armi.memory-formation.contextual'::text, 'armi.memory-revision.contextual'::text]))),
