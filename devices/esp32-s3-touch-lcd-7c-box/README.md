...
离线脸使用 `#3A3F47` 暗灰色和 180/255 的图形不透明度，并关闭呼吸效果，因此明显暗于青色演示；离线切换没有调低屏幕背光。目前尚未实施提亮离线脸的修改。
...
上述均为独立设备验证，没有启动 ARMI Runtime；Runtime 自动重连、真实心情同步、精确色差/亮度与撕裂尚未验收。未运行宿主 C 测试（未配置宿主 C 编译器）。单张情绪造型仍可继续按操作者反馈调整。
