...
冻结资料格式与模型可读文本分离：旧提示词格式不要求旧数据兼容分支，也不需要重建主体数据库。当前对方的 Creator 身份依据 owner 提供的 `sender_party_kind`，不能把场合的主要对方 `primary_party_id` 当作 Creator；非创造者私聊也会以当前对方为场合主要对方。非创造者对话的回复、事件评价和关系变化共用 2048 token 输出预算，与普通 Creator 对话一致。Ark 对 `armi.other-human-dialogue-candidate.v9` 使用完整 JSON Schema 引导（`strict: false`）：2026-09-20 同输入对照中，供应商 strict 解码重复拼接或错误选择结束会话，关闭后返回通过完整 Schema 的正常回复。ARMI 的 JSON、合同、引用、owner 与原子提交校验不放宽，也不截取或修补非法返回；其他合同保持原设置。当前通用候选合同确实要求的 `candidate_base`、显式受托任务的来源 ID，以及专项反思的目标版本，仅在相应用途中单列提交字段；反思只有目标状态保留重建完整候选所需的 schema 字段，其余来源 ID、版本和摘要不重复展示。所有程序绑定、校验及原子提交语义保持不变。
