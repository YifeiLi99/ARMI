...
自主检查在完成 Mood/Mind 联合 Jev 评价并重新冻结 Context 后，独立调用一次 Jev Choice 判断是否值得进入完整认知，不调用主模型。输入包含有界的自我、心情、动机、关注、活动、近期对话与能力投影；不按对象数量或本地资格阈值直接决定是否启动。`engage` 创建后续认知，`wait` 明确不调度，`unknown` 作为无法判断的失败保存，不能冒充主体选择等待。消费 Jev 的 choice，不用最大概率替换或另设置信度否决。检查不形成行动意图、不写主体心理状态，具体做什么仍由后续完整认知决定。
