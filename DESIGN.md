...
文本主认知只允许 Qwen 或 DeepSeek，默认 Qwen3.8-Flash，方舟不再属于主模型选择或回退路径。两家均通过 OpenAI SDK 使用官方 Responses 接口，`reasoning.effort:none`。Qwen Responses 当前未列出 JSON 格式约束参数，因此只通过提示词要求 JSON，并显式设置 `store:false`；不发送可能被忽略的格式参数。DeepSeek 使用官方 `text.format.type:json_object`，不发送 `strict` 或服务端 Schema。两家的原认知提示后追加同一份完整候选生成 Schema，每种对话只提供一个经该 Schema 验证的完整层级示例；普通他人示例同时展示评价、经历和聊天偏好事实，boundary 与 commitment_change 为 null。无经历或仅有经历但无关系变化的形式由 Schema 与回归用例覆盖。无关系变化必须将 relationship_change 整体置 null，不能填写全空对象；承诺引用只能指向冻结 Context 中的关系承诺，普通调侃不自动形成承诺冲突。模型侧生成控制可以不同，后端仍使用同一套严格 Schema 与领域校验，不修补非法 JSON、不放宽字段、不自动重试或切换模型。出现格式问题应检查请求、原始返回和结构化输出适配，不能关闭后端校验。原 Context、purpose 合同、Subject Commit 和 Effect 链保持不变，单轮仍只有一次生成。

DeepSeek 的 JSON Output 指南同时要求 JSON 指令与输出格式样例，完整 Schema 不能代替样例。两家 Creator cognitive act 与普通他人对话共同提供完整的非空评价样例：`candidate.appraisal` 保存事件，内部 `appraisal` 保存评价维度。层级说明使用完整路径，并从当前 Schema 的 properties 生成 candidate 直接字段清单，禁止将事件引用 `candidate.appraisal.basis_refs` 再复制为 `candidate.basis_refs`；其他嵌套字段也不得提升到 candidate 层。样例使用本轮可用引用并明确仅示意格式，不要求复制判断、引用或形成评价；样例与额外字段拒绝行为由原后端 Schema 回归验证。非思考聊天采样按供应商独立设置：Qwen 使用 `temperature:1.0`，不显式传 `top_p`；DeepSeek 使用 `temperature:1.0`、`top_p:1.0`（官方非思考模式固定值）。这是经用户选定的聊天默认值，不代表不同模型具有相同随机性或保证 JSON 正确；不能以降低温度替代严格后端 Schema 校验。Responses 生成方式、关闭思考及单次调用保持不变。排查格式错误时对照原始 HTTP 文本块与 SDK 提取值，不能将模型侧 JSON 模式误当成候选已验证。

聊天生成使用完整嵌套评价示例与多行 JSON，明确区分事件意义 concerns[].significance、动机重要性 mind_appraisals[].significance 和 self_involvement 的枚举；准则评价的 scope 只属于冲突分支。普通他人关系变化的生成 Schema 从原后端合同选择带非空 interpretation 的单一对象分支（或整体 null），保留事实、边界与承诺能力；已有关系理解未变时可保留原解释，不强制产生新判断。两家共用这一较窄生成视图，后端原 Schema、版本与领域校验不变，不删除非法字段或自动补解释。该约束避免首次建关系选择仅事实分支后再被 owner 拒绝。
...
生成提示按实际 Schema 的 required 区分必填与可省略，nullable 不等于可省略；无变化使用合同允许的 null 或空数组，不为补齐字段编造状态。生成 Schema 是从后端合同派生的较窄视图，不称为与后端完全相同。普通他人边界中 contact/exit 会阻止回复；address 仅为称呼限制，privacy/disclosure 分别为隐私与披露限制。“别拿这件事开玩笑”“先别给建议”等仍允许聊天的偏好保存为对方表达事实及关系理解，不扩大为停止联系，不自动生成承诺。提示与字段描述解释已有行为，后端合同版本及领域校验保持不变。DeepSeek 显式使用官方 `tool_choice:none`，只生成候选消息；这不保证消除非法 JSON 或 DSML 标签，仍严格拒绝异常返回，不做截取或修复。
