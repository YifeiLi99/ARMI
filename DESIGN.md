...
自主检查在完成 Mood/Mind 联合 Jev 评价并重新冻结 Context 后，独立调用一次 Jev Choice 作前置大类分类，不调用主模型。一次回答同时决定是否启动和投入方向：`continue_activity`（继续活动）、`explore`（探索与思考）、`communicate`（主动交流）、`reflect`（回顾与整理）、`wait`（暂不行动），另保留 `unknown`（无法判断）。输入包含有界的自我、心情、动机、关注、活动、近期对话与能力投影；不按对象数量或本地资格阈值直接决定是否启动。四种行动方向创建后续认知，`wait` 明确不调度，`unknown` 作为无法判断的失败保存，不能冒充主体选择等待。消费 Jev 的 choice，不用最大概率替换或另设置信度否决。

Attention 在结算检查的同一事务内，把分类保存为来源和后续机会的 `autonomy_category`；它是调度方向，不是心理状态、事实或行动权限。后续 Context 带入这个冻结方向，主模型使用对应任务说明来细化具体内容，不重新自由选择大类；若条件不足仍可不行动、延期或需要信息。继续活动须围绕已有活动，探索围绕有依据的兴趣与疑问，交流仍核验出口与目标，回顾整理不直接触发睡眠维护或绕过记忆写入条件。各类共用原有认知、Owner 校验、Subject Commit 和 Effect，不因分类引入另一套执行机制。MCP/CLI 自主历史和 Creator 自主记录显示分类，诊断事件保留 choice 与概率。

60 秒是内部考虑间隔，不是主动发言间隔。轻判保留最近两条对话的时间、来源用途、后续是否有人类输入及至多 2048 字正文，避免截掉失败报告末尾的等待条件。活动 ready 不证明当前已有可推进的一步；旧失败、未变化的障碍及刚表达过的等待理由不构成反复启动或联系的依据。完整认知的 expression 是立即发给对方的消息，内部的等待、安静或不重复决定应省略 expression；已有活动受阻时记录 wait 和恢复条件，不只 defer 却继续保持 ready。普通表达风格仅在已决定发送时适用。
...
自主检查与后续完整认知复用来源事件的 Mood/Mind 共享 Jev 回执，不重复心理刺激或心理评价计费；自主 Choice 是独立的一次调用。检查只提交 Attention 调度；符合条件时原子创建唯一后续认知，并重新核验主体版本、冻结 Context。未调度不等于主体决定沉默。
