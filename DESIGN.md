...
`autonomy_plans` 只存阶段、调度、退避和最近判断关联；两段用量从 `provider_usage_calls` 汇总，不建第二份计数账。CLI/MCP、管理状态及 Creator 页面显示相同合同。数据库 v30 提供精确 v29→v30 前向升级：保存主体、记忆、关系和调用历史，删除额度登记表，取消旧未完成自主机会，并同步 Runtime 配置 v5、模型绑定 v4；升级不重复出生、不改凭据内容。本机部署另行执行。
...
历史 `system_notifications` 与关联 Effect 的读取、数据权利和中断合同保留；新失败不再登记此类通知。
...
当前 baseline 为 v31，支持 v21–v30 精确前向升级。v30→v31 将动作意图及其唯一修订合为 action_intents，保留动作 ID、正文引用、认知来源和已发生的 Effect；历史存在多修订或不完整指针时明确拒绝，不丢弃历史。v27→v28 接受新增的文本/语音 v7 决策合同，保留历史候选与制品。其中 v25→v26 分别追加 Mind v4 / Mood v4 的 module_migration revision，保留旧 ID、payload、来源和情绪评分；不重复出生、不重算旧情绪。Mind 新动机为空，不补造历史需求。旧 Mood 评价只为轨迹比较读取，旧候选不执行。表、Owner 和权限不变；升级事务核验来源摘要及目标结构，失败完整回滚。安装更新由明确授权的签名验收包流程执行，部署事实与真实心理行为验收分别记录。
