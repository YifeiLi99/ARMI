...
心理事件入口为 `event.appraise → cognition.context.prepare → cognition.execute`。每个事件只发送一批 Mood 25 题与 Mind 18 题，评价同一事件和冻结旧状态，Mood/Mind 各自计算并提交；主认知读取新状态，无 Mood/Mind 写入能力。关注、动机和活动只作为背景，不展开额外题目。Codex 返回是新事件，再走一轮相同流程；本轮回复本身不重新触发评价。
...
Attention 持有唯一调度状态。没有待处理人类输入、活动认知、实时语音或睡眠维护，回复 Effect（含待执行状态）已结束且安静满 60 秒后，创建 `consider_autonomy_check`。正常检查、等待和延期均以 60 秒为间隔，不因连续不行动延长；技术失败的退避保持独立。人类输入、工具结果、到期活动与关切可缩短等待，信号按来源版本消费，轻判启动至少相隔 60 秒；普通工作完成与时钟刷新不触发额外认知，不补跑积压轮次。

自主检查直接读取已有状态，独立调用一次 Jev Choice，只判断 `wake`（唤醒认知）或 `wait`（继续等待），另保留 `unknown`（无法判断）。检查不执行 Mood/Mind 心理评价，不选择行动。`wake` 创建后续认知，`wait` 不调度，`unknown` 作为无法判断的失败保存。消费 Jev 的 choice，不用最大概率替换或另设置信度否决。

Attention 在结算检查的同一事务内，把唤醒结果保存为来源和后续机会的 `autonomy_category`。后续完整认知直接读取现有 Mood/Mind，自主决定做什么，也可不行动、延期或需要信息；不再次执行心理评价。唤醒不授予行动权限，仍共用 Owner 校验、Subject Commit、Effect 及渠道核验。MCP/CLI 和 Creator 自主记录显示唤醒结果。

60 秒是内部考虑间隔，不是主动发言间隔。轻判付费输入仅投影当前心情数值、已有动机强度与优先级、机会、活动状态和出口状态；不发送历史正文、题库、身份或来源 ID。来源版本保留在冻结 Context 中供陈旧核验。目标输入预算为 500 token，请求另有 1024 字节增长上限；字节上限不等同于 Provider token 计数。活动 ready 或时间流逝本身不构成唤醒理由。完整认知的 expression 是立即发给对方的消息，内部等待或不重复决定应省略 expression；已有活动受阻时记录 wait 和恢复条件。
...
心理评价在发送前保存实际 HTTP 请求正文，收到响应后先保存原始响应正文，再解析评价。正文保存在该次 `event_appraisals.context_document` 的 `provider_request`、`provider_response`，不保存 HTTP 头或凭据，不写入常态日志；数据权利清理随该文档一并删除。请求已保存但响应缺失表示未取得可留存响应，不能补造成功结果。

产品约定：真实对话、自我产生的新事件及外部信号均须经过一次心情变化判断，不能用是否回复或行动筛掉评价。自主定时检查、唤醒和决定做什么只是读取已有心理状态，不属于新的心理事件，不运行心理评价。

每次判断都要有明确结果：需要更新心情评价，或经判断无需更新。心情不变是合法结果，不要求每次产生新的情绪、增加刺激或改写心情数值。候选字段缺失、未执行、失败或结果未知不能充当“已经判断不变”；重复思考旧事仍须判断，但不能据此重复叠加同一刺激。

正式链路为 `event.appraise → cognition.context.prepare → cognition.execute`。心理评价 Jev 适配器每个事件只调用一次，固定 `jev-1.13.0`，失败或中断不回退；完成心情提交后重新冻结认知 Context。同一来源与版本复用心理评价，轮询及 Mood 通知不产生事件。自主检查在 `cognition.execute` 中另作一次 Jev Choice，职责与回执均独立于心理评价。
...
Mood 以 Scherer 的 16 个评价维度组织每事件固定 25 道离散问题；已有目标作为背景，不逐目标复制题目。收益、损失及刺激正负性质分别保留。unknown、不适用与明确无影响分别表示。Jev 返回概率和置信度独立保留，不作为情绪强度。完整题目、锚点、研究依据见 [心情设计正文](docs/02-系统设计/05-情绪、心情与私有心情窗.md)。

正式题库采用中文对象限定提示：每题独立携带主体、评价对象和问题，self 固定指 ARMI；引语说话者不自动成为主体。报告事件与当前询问、提议、玩笑区分，收益、证据和阶段共用对象规则；已有目标的进度预测仍保留未发生结果的阶段。核验背景不因转述而失效，无损与维持原状不自动等于充分获益，未知时限不等于无压力。
...
`consider_autonomy_check` 只作独立 Jev 唤醒判断，没有主模型候选或主体提交；等待时记录 `not_scheduled`。它与 `consider_autonomous_life` 均直接进入 Context 准备，不排入 `event.appraise`。保留至少 60 秒间隔、输入与发送占用、睡眠及维护限制。完整认知正常路径一次主 LLM；普通文本既有格式重试合同不变。
...
Codex 结果事件的 Jev 输入只描述已核验的委托结果状态；完整返回正文保留在 Evidence，供后续认知读取，不进入 Jev 的事件文本或背景。一般事件的正文已位于 `event.content` 时，背景不重复附带 `current_evidence`；Runtime 身份、当前 purpose 和能力目录等机器元数据不作为心理评价材料。

每个事件的 Mind 评价只绑定该事件来源，固定评价 16 个状态维度及关联状态、考虑机会两题。Context 的关注、动机、活动与到期复查只提供背景依据，不再从这些条目挑选额外评价对象；四项注意窗口仅属于主认知的背景选择，不是 Jev 题目倍增器。联系缺口没有依据时返回 unknown，确知不适用时返回 not_applicable，不由活动身份创造联系人。
...
Context 使用同一能力快照生成目录和候选 Schema，关闭能力没有模型可执行分支；已开启但暂不可用时显示状态和原因，执行仍由责任 Owner 验证。轻量唤醒只读取出口状态，完整认知才使用能力目录；唤醒不选择或授权能力。主动表达与行动分离，沿用一至三条消息规则、Expression／Effect 和渠道核验。没有回应、近期联系和时间属于可供判断的事实，不能机械禁言。QQ 出口只能使用有效的 Creator 绑定，不自动回退网页；联系边界和数据权利保持有效，unknown 不重放。
...
诊断是管理证据，不是业务事实、主体记忆或权限来源。Kernel 的 `diagnostic_scope` 用 contextvars 关联既有请求、work、episode、attempt、call、effect 等身份，`record_diagnostic` 通过标准 Python logging 发出事件。Runtime Foundation 负责落盘、脱敏、标准日志与未处理异常接入、查询；业务模块不依赖 Runtime 私有记录器。数据库只在事务异常或取消时于归还连接后记录结果，正常事务不逐次落日志；业务提交由 owner 的事件追踪。日志 I/O 不进入权威写事务。
...
INFO 表示有诊断价值的阶段及主体不行动/取消结果；WARNING 表示降级、重试、延期与可恢复异常；ERROR/CRITICAL 表示失败、未知结果及崩溃。标准 INFO 及以上接入共同记录器，httpx/httpcore 的普通传输日志除外，其 WARNING 及以上仍保留；Provider owner 保留真实调用结果与失败。DEBUG 默认关闭。正常心跳不落日志，仅记录权限挂起、恢复与丢失；自主调度只记录实际接纳，睡眠维护的未到期、静候、安全点等待与重复待办不反复记录，实际 Jev 决策与维护进展仍保留。内部错误码继续供业务判断，外部故障先记录原始异常再转换。收到 HTTP 4xx/5xx 与网络超时分别结算 Provider 回执，未知费用保持未知。本机制不改变重试次数、超时或自主决策。
