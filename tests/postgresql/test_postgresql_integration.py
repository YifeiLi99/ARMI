    ...
    def _assert_supported_database_upgrade(self, source_version: str) -> None:
        ...
        with psycopg.connect(old.migrator_dsn) as connection:
            ...
            connection.execute(
                ...
                (observation_attempt_id,web_observation_request_id,work_id,work_attempt_id,work_lease_token,attempt_no,binding_id,credential_identity,dispatch_state,provider_model_id,input_tokens,output_tokens,web_search_calls,estimated_cost_microyuan,result_status,error_code,dispatched_at,settled_at,usage_contract_version)
                VALUES (%s,%s,%s,%s,1,1,'armi.model-tool.volcengine-ark-web-search-v1',%s,'settled','doubao-seed-evolving',12,3,1,157,'failed','WEB-TEST',statement_timestamp(),statement_timestamp(),0)""",
                (
                    historical_call,
                    historical_request,
                    historical_work,
        ...
        with psycopg.connect(old.migrator_dsn) as connection:
            ...
            self.assertEqual(
                connection.execute(
                    "SELECT to_jsonb(r) FROM armi.mind_revisions r WHERE subject_id=%s AND mind_version<=2 ORDER BY mind_version",
                    (born.subject_id,),
                ).fetchall(),
                old_mind_history,
            )
            self.assertEqual(
                connection.execute(
                    "SELECT count(*) FROM armi.subject_component_revisions WHERE component_kind='mind'"
                ).fetchone(),
                (0,),
            )
            ...
            self.assertEqual(
                connection.execute(
                    "SELECT mind_revision_id,mind_version,semantic_payload "
                    "FROM armi.mind_revisions WHERE mind_revision_id=%s",
                    (old_mind[0],),
                ).fetchone(),
                old_mind,
            )
            migrated_mind = connection.execute(
                "SELECT r.origin_kind,r.previous_revision_id,r.mind_version,r.semantic_payload "
                "FROM armi.mind_heads h JOIN armi.mind_revisions r "
                "ON r.mind_revision_id=h.current_revision_id "
