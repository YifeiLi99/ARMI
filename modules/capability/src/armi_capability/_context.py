"""Capability-owned Context projection payloads."""
...
from .api import CapabilityContextStatePayload

_MAX_JSON_SAFE_INTEGER = (1 << 53) - 1


async def load_context_state_payloads(
    ...
    *,
    ...
    rows = await (
        await transaction.execute(
            """
            SELECT capability.capability_id, capability.capability_kind,
                   capability.operation_class, capability.availability_status,
                   capability.configuration_version,
                   request.capability_request_id, request.request_version,
                   CASE WHEN request.current_status IN ('granted', 'limited')
                         AND permission.valid_until <= statement_timestamp()
                        THEN 'expired' ELSE request.current_status END,
                   request.resolution_reason_class, request.audience_scope,
                   request.data_scope, request.purpose, request.workspace_scope,
                   request.artifact_scope, request.network_access,
                   request.requested_valid_for_seconds,
                   request.requested_max_uses,
                   request.requested_max_payload_bytes, permission.grant_id,
                   CASE WHEN permission.status = 'active'
                         AND permission.valid_until <= statement_timestamp()
                        THEN 'expired' ELSE permission.status END,
            ...
            LEFT JOIN LATERAL (
                SELECT item.* FROM armi.capability_requests AS item
                WHERE item.subject_id = %s
                  AND item.capability_id = capability.capability_id
                ORDER BY item.created_at DESC, item.capability_request_id DESC
                LIMIT 1
            ) AS request ON true
    ...
    return tuple(_state_payload(row) for row in rows)


def _state_payload(row: tuple[object, ...]) -> CapabilityContextStatePayload:
    ...
    if request_id is not None:
        ...
        if source_version > _MAX_JSON_SAFE_INTEGER:
            raise ValueError("capability context source version is not JSON-safe")
    return cast(UUID, row[0]), source_version, payload, request_status or "unauthorized"


__all__ = ("load_context_state_payloads",)
