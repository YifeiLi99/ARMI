...
_ANCHORS: dict[MindVariable, tuple[str, tuple[str, str, str, str, str]]] = {
    ...
    MindVariable.COMPETENCE_FRUSTRATION: (
        "在该对象的具体任务或挑战中，主体的能力与方法是否确实无法有效应对？须有应对失效的具体证据。被迫做不愿意的事属于意愿冲突，不自动证明能力受挫；任务已准确完成且方法完全掌握时，不因无法拒绝命令而判应对失效。不将外部技术故障解释为能力缺陷。只有意愿冲突而未交代任务能力情况时选 unknown；缺少受阻证据不等于证实没有受阻，零档须有当前能有效应对或明确无能力障碍的依据。",
        (
            "有正面证据证明当前方法能有效应对，或明确说明无自身能力障碍；仅未提及能力情况不能选本项",
            "主体能力或方法存在局部小困难但尚能应对；外部服务故障不属于这种困难",
            "多次尝试仍有具体障碍",
            "主要应对方法失效且明显受阻",
            "现有可用方法均无法应对核心挑战",
        ),
    ...
    MindVariable.COMPREHENSIBILITY: (
        ...
        (
            "存在明确的待理解内容，且有证据证明它完全不可理解、没有任何切入点；材料没交代内容或知识条件时选 unknown",
            "只有很弱的片段线索",
            "有部分可理解结构和方法",
            "主要结构可理解且有明确方法",
            "完全具备理解条件，只需补齐具体信息",
    ...
    MindVariable.LEARNING_PROGRESS: (
        "本次 ARMI 是否获得新理解、新线索或新方法？只根据学习结果证据选档，尝试失败仍可能学习。仅报告工具或平台故障，未交代学习情况时选 unknown，不能默认没有学习。泛泛夸奖或未提及学习结果同样选 unknown；没有学习证据不等于证实没有学习。只有明确说本次没有新增理解或方法进展才选零档。",
        (
            "明确无新增理解或方法进展",
            "获得一个很小线索",
            "排除部分假设或理解部分规律",
            ...
            questions[f"mind_{index}_{variable.value}"] = {
                ...
                "criteria": {
                    **{f"level_{i}": text for i, text in enumerate(levels)},
                    ...
                    else "只交代意愿冲突、讽刺或强迫，未交代完成任务的能力与方法是否受阻；或其他未提供能力状况的材料。没有能力失败证据不证明能力没有受阻，应选本项。"
                    if variable == MindVariable.COMPETENCE_FRUSTRATION
                    else "材料没有说明谈论的内容、可用知识或理解条件，例如只有‘还行吧’且不知道在谈什么。不能因此断定内容不可理解，应选本项。"
                    if variable == MindVariable.COMPREHENSIBILITY
                    else "没有提供 ARMI 对具体活动是否认同、是否有意义的证据；例如只有转发内容或他人指令，没有自己的活动意义事实。未提供意义不是明确无意义，应选本项。"
                    if variable == MindVariable.MEANING
                    else {
