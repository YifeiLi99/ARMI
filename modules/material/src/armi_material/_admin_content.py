...
class PostgreSQLMaterialAdminContent:
    ...
    def apply(
        ...
        row = cast(
            tuple[Any, ...] | None,
            tx.execute(
                "SELECT r.life_material_revision_id,r.revision_no,r.deleted_at,r.artifact_id,r.title,r.metadata,r.material_status,r.material_kind,r.owner_party_id,r.material_created_at FROM armi.life_material_revisions r WHERE r.life_material_id=%s AND r.subject_id=%s AND r.is_current",
        ...
        if row is not None:
            changed = tx.execute(
                ...
                (row[0], command.expected_version),
            )
            if changed.rowcount != 1:
                raise AdminContentViolation("ADMIN-CONTENT-VERSION-CONFLICT")
        tx.execute(
            "INSERT INTO armi.life_material_revisions (life_material_revision_id,life_material_id,revision_no,previous_revision_id,admin_change_id,artifact_id,title,metadata,revision_kind,privacy_status,material_status,source_kind,subject_id,material_kind,owner_party_id,material_created_at,deleted_at) "
            "VALUES (%s,%s,%s,%s,%s,%s,%s,%s::jsonb,%s,%s,%s,'administrator',%s,%s,%s,COALESCE(%s,statement_timestamp()),CASE WHEN %s THEN statement_timestamp() END)",
            (
                revision,
                command.object_id,
                version,
                ...
                status,
                context.subject_id,
                values["material_kind"] if row is None else row[7],
                context.subject_party_id if row is None else row[8],
                None if row is None else row[9],
                deleted,
            ),
        )
        return {
