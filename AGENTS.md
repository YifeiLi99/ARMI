...
- QQ/NapCat 默认关闭且按需安装。设置一键入口与 CLI/MCP setup 共用组件准备用例，自动处理下载校验、端口与独立通信密钥；不要求人工复制 token 或单独安装 QQ。启用需要双方 QQ 号，扫码与平台安全验证由用户完成。受管 Node 随环境启停，配置和数据留在所属环境；不接管用户自行运行的 QQ，不把安装验证表述为真实登录或消息闭环通过。
